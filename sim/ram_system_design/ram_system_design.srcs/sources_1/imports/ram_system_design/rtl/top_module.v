...
module top_module 
   (
      ...
      input              i_error_en      ,  // NEW: control pulse from TB to enable error injection
      // ---------- Output signals ---------
   ...
   */
    ...
    */
      ...
      // =========================================================================
      // ERROR INJECTION LOGIC (Controlled by Testbench)
      // =========================================================================
      wire [31:0] ram_write_data;
      
      // Inject errors ONLY IF i_error_en is High
      assign ram_write_data = (i_error_en && (write_address == 6'd10 || 
                                              write_address == 6'd20 || 
                                              write_address == 6'd30)) ? ~write_data : write_data;
      // =========================================================================
      
    // IP RAM INSTANTIATION
    blk_mem_gen_0 ram_inst (
              .clka(i_clk),             // input wire clka
              .ena(1'b1),               // input wire ena
              .wea(wea),                // input wire [3 : 0] wea
              .addra(ram_address[5:0]), // input wire [5 : 0] addra
              .dina(ram_write_data),    // INPUT MODIFIED FOR DYNAMIC ERROR INJECTION
  ...
  always @ (posedge i_clk) begin
   ...
   // Triggering generate_rd_addr exactly on start_checker.
   always @ (posedge i_clk) begin
      if (i_rst) begin
  ...
  assign wea = {4{write_data_valid}};
  assign ram_address = write_data_valid ? {26'b0, write_address} : {26'b0, read_address};
